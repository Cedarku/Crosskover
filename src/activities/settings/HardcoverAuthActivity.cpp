...
void HardcoverAuthActivity::onWifiSelectionComplete(const bool success) {
  if (!success) {
    {
...
void HardcoverAuthActivity::performAuthentication() {
  int64_t userId = 0;
  std::string username;
  const auto result = HardcoverSyncClient::authenticate(userId, username);

  {
    RenderLock lock(*this);
    if (result == HardcoverSyncClient::OK) {
      HARDCOVER_STORE.setIdentity(userId, username);
      HARDCOVER_STORE.saveToFile();
      state = SUCCESS;
      statusMessage = std::string(tr(STR_AUTH_SUCCESS)) + (username.empty() ? "" : (": " + username));
    } else {
      state = FAILED;
      errorMessage = HardcoverSyncClient::errorString(result);
    }
  }
  requestUpdate();
}
...
void HardcoverAuthActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_HARDCOVER_AUTH));
